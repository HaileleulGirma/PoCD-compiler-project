// added lookup functionality for the symbol table 
#include <iostream>
#include <fstream>
#include <cctype>
#include <string>
...
public:
    ...
    float getVariable(const std::string& name) const {
        ...
        if (it != symbolTable.end() && !it->second.empty()) {
            ...
            return std::stof(it->second.back().value);
        } else {
            std::cerr << "Error: Variable '" << name << "' not found." << std::endl;
            exit(EXIT_FAILURE);
        }
    ...
    std::vector<Token> getSymbolEntries(const std::string& tokenValue) const {
    auto it = symbolTable.find(tokenValue);
    return (it != symbolTable.end()) ? it->second : std::vector<Token>();
    }
...
do {
    std::cout << "\nSymbol Table Lookup Menu:\n";
    std::cout << "1. Print Symbol Table\n";
    std::cout << "2. Lookup Variable\n";
    std::cout << "3. Clear screen\n";
    std::cout << "4. Return to Main Menu\n";
    std::cout << "Enter your choice: ";
    std::cin >> lookupChoice;
...
case 2: {
    std::string variableName;
    std::cout << "Enter the variable name to lookup: ";
    std::cin >> variableName;

    
    const auto& symbolEntries = lexer.getSymbolEntries(variableName);
    if (!symbolEntries.empty()) {
        std::cout << "Symbol Table Entries for Variable '" << variableName << "':\n";
        for (const Token& token : symbolEntries) {
            std::cout << "\tType: " << getTokenTypeName(token.type)
                      << ", Value: " << token.value << ", Line Number: " << token.lineNumber << std::endl;
        }
    } else {
        std::cout << "Variable '" << variableName << "' not found in the symbol table.\n";
    }
    break;
}
